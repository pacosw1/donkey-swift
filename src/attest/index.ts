...
export class AttestService {
  private challengeTtlSec: number;

  constructor(
    private db?: AttestDB,
    private cfg?: AttestConfig
  ) {
  ...
  async verifyAssertion(
    ...
    let stored: { keyId: string; publicKey: string };
    try {
...
function decodeCBORAttestation(data: Buffer): AttestationObject {
  ...
  const obj = readValue() as Record<string, unknown>;
  if (!obj || typeof obj !== "object") throw new Error("expected CBOR map");

  return {
    fmt: obj.fmt as string,
    attStmt: {
      x5c: (obj.attStmt as Record<string, unknown>)?.x5c as Buffer[] | undefined,
      receipt: (obj.attStmt as Record<string, unknown>)?.receipt as Buffer | undefined,
    },
    authData: obj.authData as Buffer,
  };
}
