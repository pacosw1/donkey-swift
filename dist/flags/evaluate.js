 ...
 */
export function resolveAttr(ctx, path) {
    if (!path)
        return undefined;
 ...
 */
function bucketOf(seed, buckets) {
    if (buckets <= 0)
        return 0;
 ...
 */
function evalCondition(cond, ctx, flagKey) {
    switch (cond.op) {
        case "and":
 ...
 */
function pickServe(serve, flag, ctx) {
    switch (serve.kind) {
        case "value":
 ...
 */
function pickWeightedVariant(variants, flagKey, userId) {
...
export function evaluateFlag(flag, ctx) {
    ...
    for (const rule of rules) {
        if (!evalCondition(rule.condition, ctx, flag.key))
            continue;
